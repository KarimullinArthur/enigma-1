...
class Roter:
    def __init__(self, number: int, position: int, turn_position: int):
        MAX_CHR = 1114111
        ...
        for c in range(0, MAX_CHR+1):
            self.commutation_table[c] = random.choices(full_char, k=len(full_char))
...
#                     print(roter.turn_position)
    ...
    def decrypt(self, value: str):
        ...
        def dec(self):
#             result = self.roter_2_com[self.roter_2 - self.roter2_start]
#             result = self.roter_1_com[self.roter_1 - result]
#             return result
            return self.roter_1_com_inv[ord(char)]-self.roter1_start-self.roter2_start

        for char in value:
            try:
                result.append(chr(dec(self)))
            except ValueError:
                print('PINGD', end='')
                self.roter1_start = 0
                self.roter2_start = 0
